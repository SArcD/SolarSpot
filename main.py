...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Definir los valores iniciales de los deslizadores para los umbrales
        default_threshold_min = 55
        default_threshold_max = 53

        # Agregar deslizadores para los parámetros de umbralización
        threshold_min = st.slider("Valor de umbral mínimo", 0, 255, default_threshold_min)
        threshold_max = st.slider("Valor de umbral máximo", 0, 255, default_threshold_max)
        ...
        # Aplicar umbralización adaptativa para detectar las manchas solares dentro del disco solar
        binary_manchas_solares = cv2.adaptiveThreshold(imagen_gris, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, threshold_min, threshold_max)
